...
function DriverRideHistory({ navigation }) {
    ...
    const fetchCompletedRides = useCallback(async () => {
        ...
        try {
            const response = await axios.get(`${Config.apiBaseUrl}/completed-rides`, {
                headers: {
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View style={styles.card}>
                        <Text style={styles.text}>Passenger Name: {item.passengerName}</Text>
